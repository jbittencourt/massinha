...
//lista dos erros possíveis no ambiente
 ...
 * @const RD_FINDER_MODO_OCUPADO Ocupado é visivel mas não pode receber mensagens
...
class RDFinder extends RDFerramenta {
  /**
   * @var integer $modo Guarda o modo de operação atual do finder(normal, oculto, ocupado)
   * @var array $chats_abertos Guarda todos os chats que estão atualmente abertos no sistema
   */
   ...
   *
   * Obtém a partir da do objeto ambiente que deve estar registrado
   * na seção quais são os usuários atualment conectados
   *
   * @return mixed  Retorna com uma RDLista dos usuários atualmente conetados ou um RDError
  ...
  /**
   * Envia um mensagem para um usuário conectado
   *
   * @param string $mensagem Mensagem a ser enviada
   * @param integer $para Código do usuário para quem se deseja enviar um mensagem
   */
  ...
  /**
   * Muda o modo do usuário.
   *
   * O modo do usuário deve estar dentro do array $this->modos. Ele faz a alteracão necessária
   * no campo visibilidade da tabela sessao_ambiente(RDSessaoAMbiente). A secao do usuário está 
