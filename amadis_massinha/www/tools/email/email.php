...
$_SESSION[AMADIS_ESCOLA][EMAIL][COMPOSE]="";
...
if(isset($_REQUEST[acao])) {
  ...
  case "A_change_email":
    ...
    $pag->add("<br><br><div class=\"comum\">");
    $temp = str_replace("{EMAIL}",$_SESSION[usuario]->strEMail,$lang[email_desabilitado_desc]);
    $pag->add($temp);
    $pag->add("</div>");

    $pag->add("<p><div align=center><a href=\"$_SERVER[PHP_SELF]?acao=A_change_email_make\" class=\"comum\">$lang[criar_novo_email]</a></div>");
...
if(!empty($mens)) {
  foreach($mens as $men) {
    ...
    $pag->add("<td class=\"$class\" width=50%><a href=\"vemen.php?frm_idMen=$men[uid]&$mailbox\" class=\"comum\">$men[subject]</a></td>");
